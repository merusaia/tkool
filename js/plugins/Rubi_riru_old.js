...
var ruby_dic = [
  ...
  ["存在","たましい"],["レヴェル","カルマ"],["天子音","てんしょん"],
  ["憑依","ひょうい"],["鏡を見る","ステータス"],
  ["審判の時へ","タイトルへ"],["だが断る","やめる"],["現実世界","げんじつせかい"],
  ["並大抵","なみたいてい"],["精進","しょうじん"],["根性","こんじょう"],["満身創痍","まんしんそうい"]

// 装備品など。
  ["浄化","じょうか"],["穢れ","けがれ"],["範囲","はんい"],
...
Window_Base.prototype.drawText = function(text, x, y, maxWidth, align) {//再定義
    // ↓merusaiaが追加。他のプラグインと競合するので、他のプラグインで上書きされたくない部分をここにコピペして持ってくる。
    
    // 1. サンシロさんのプラグイン: SAN_EntireEscapeCharacter.jsのWindow_Base.prototype.drawText
    ...
    //ここriruさんの文字表示とかぶるので、この一行だけコメント？ Sanshiro.EntireEscapeCharacter.Window_Base_drawText.call(this, text, x, y, maxWidth, align);
    ...
    if(p_Jisage==0||(p_Jisage==2&&$gameMessage.ruby_e_hantei(textState.text)))textState.y += 6+ruby_c_size;//字下げ
          hankaku = new RegExp("([A-Z]|[0-9]| |[ｦ-ﾟ])","img");//半角が含まれているもの
