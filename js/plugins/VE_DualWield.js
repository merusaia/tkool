...
(function() {
	...
	DataManager.loadDatabase = function() {
		...
		PluginManager.requiredPlugin.call(PluginManager, 'VE - Dual Wield', 'VE - Basic Module', '1.15');		
	};
...
/*:
 * ------------------------------------------------------------------------------
 * @plugindesc v1.01 - New options for dual wield and two handed wepaons.
 * @author Victor Sant
 *
 * @param == Equip Display ==
 *
 ...
 *   An actor with this tag have the attack power of the bare handed attacks changed.
 *     X : change value. Numetic. Can be negative.
 *
 * ---------------
 *
...
(function() {
	...
	VictorEngine.loadNotetagsValues = function(data, index) {
		...
		if (this.objectSelection(index, list)) VictorEngine.DualWield.loadNotes1(data);
		var list = ['skill', 'item'];
		if (this.objectSelection(index, list)) VictorEngine.DualWield.loadNotes2(data);
		var list = ['weapon'];
		if (this.objectSelection(index, list)) VictorEngine.DualWield.loadNotes3(data);
	};
	...
	BattleManager.initMembers = function() {
		...
		this._dualWieldActions = [];
	};
	
	VictorEngine.DualWield.startAction = BattleManager.startAction;
	BattleManager.startAction = function() {
		var action = this._subject.currentAction();
		var original = JsonEx.makeDeepCopy(action);
		action.prepareDualWieldAction(1);
		VictorEngine.DualWield.startAction.call(this);
		this.addDualWieldAction(original, action, this._targets);
	};

	VictorEngine.DualWield.endAction = BattleManager.endAction;
	BattleManager.endAction = function() {
		if (this._dualWieldActions.length > 0 && !this.hasFollowUp()) {
			var dualWield = this._dualWieldActions.shift();
			this.prepareDualWieldAction(dualWield);
		} else {
			VictorEngine.DualWield.endAction.call(this);
		}
	};
	
	BattleManager.hasFollowUp = function() {
		return this._followUpActions && this._followUpActions.length > 0;
	};
	
	BattleManager.prepareDualWieldAction = function(dualWield) {
		if (dualWield.action.isDualWield()) {
			this.startDualWieldAction(dualWield);
		} else if (Imported.YEP_BattleEngineCore) {
			this._phaseSteps.push('finish');
		}
	};
	
	BattleManager.addDualWieldAction = function(original, action, targets) {
		this._dualWieldActions.push({original: original, action: action, targets: targets.clone()});
		this.removeFinishActionYEP();
	};
	
	BattleManager.startDualWieldAction = function(dualWield) {
		var subject = this._subject;
		var action  = dualWield.original;
		var targets = dualWield.targets.filter(function(target) { return target.isAlive() });
		if (targets.length > 0) {
			action.prepareDualWieldAction(2);
			if (this.veReplaceActions()) {
				this.prepaRereplaceAction(action);
				if (action._replaceActionTargets) targets = action._replaceActionTargets;
			}
			this._phase = 'action';
			this._action = action;
			...
			this.refreshStatus();
			this.starttActionYEP(targets);
			if (!Imported.YEP_BattleEngineCore) this._logWindow.startDualWieldAction(subject, action, targets);
			if (this.veFollowUpSkills()) this.addFollowUpAction(action, action.subject(), targets);	
		}
	};
	
	BattleManager.veReplaceActions = function() {
		return Imported['VE - Replace Actions'];
	}
	
	BattleManager.veFollowUpSkills = function() {
		return Imported['VE - FollowUp Skills'];
	}
	...
	Game_Action.prototype.makeDamageValue = function(target, critical) {
		...
		if (this.isDualWield()) result = Math.floor(result * this.subject().dualWieldDamage() / 100);
		return result;
	};
	...
	Game_Action.prototype.prepareDualWieldAction = function(action) {
		if (this.isDualWieldAction() && this.subject().isDualWielding()) {
			...
			this.subject().ignoreEquipment(this._dualWieldAction);
		}
	};
	...
	Game_Actor.prototype.equips = function() {
		...
		if (this._equipmentIgnored) result[this._equipmentIgnored - 1] = null;
		return result;
	};

	VictorEngine.DualWield.paramPlus = Game_Actor.prototype.paramPlus;
	Game_Actor.prototype.paramPlus = function(paramId) {
		if (this._ignoreEquipIndex) this._equipmentIgnored = 3 - this._ignoreEquipIndex;
	...
	Game_Actor.prototype.xparam = function(xparamId) {
		if (this._ignoreEquipIndex && (xparamId == 0 || xparamId == 2)) this._equipmentIgnored = 3 - this._ignoreEquipIndex;
	...
	Game_Actor.prototype.isDoubleGrip = function() {
		return this.traitObjects().some(function(data) { return data.dualWield.doubleGrip });
	};
	
	Game_Actor.prototype.isMonkeyGrip = function() {
		return this.traitObjects().some(function(data) { return data.dualWield.monkeyGrip });
	};
	
	Game_Actor.prototype.isBareHanded = function() {
		return this.traitObjects().some(function(data) { return data.dualWield.bareHanded });
	};
	
	Game_Actor.prototype.isLeftHanded = function() {
		return this.traitObjects().some(function(data) { return data.dualWield.leftHanded });
	};
	...
	Game_Actor.prototype.dualWieldModifier = function(type, base) {
		return this.traitObjects().reduce(function(r, data) { return r + (data.dualWield[type] || 0) }, base);
	};
	...
	Window_EquipItem.prototype.includes = function(item) {
		...
		if (item && this._slotId === 0 && item.etypeId === actor.equipSlots()[0]) {
			if (item.dualWield.isOffHand) return false;
			return actor.canEquip(item);
		} else if (item && this._slotId === 1 && actor.equipSlots()[1] === 1 && item.etypeId < 3) {
			if (item.dualWield.isTwoHanded) return false;
			if (item.dualWield.isMainHand)  return false;
			if (keep && actor.hasTwoHandWeapon(item)) return false;
			return actor.canEquip(item);
		} else {
	...
	Window_EquipItem.prototype.setSlotId = function(slotId) {
		if (slotId > -1 && slotId < 2 && this._actor && this._actor.isLeftHanded()) slotId = slotId === 1 ? 0 : 1;
	...
	Window_EquipStatus.prototype.drawDualWieldParameters = function(i) {
		if (i < 2 && this._actor.isDualWield()) {
			...
			if (this._tempActor) this._tempActor.ignoreEquipment(index);
		} else {
	...
	Scene_Equip.prototype.onSlotOk = function() {
		...
		if (Imported.YEP_EquipCore && actor && actor.isLeftHanded() && slotId > -1 && slotId < 2) {
			slotId = slotId === 1 ? 0 : 1;
...
if (Imported.YEP_EquipCore) {
	...
	VictorEngine.DualWield.refresh = Window_StatCompare.prototype.refresh;
