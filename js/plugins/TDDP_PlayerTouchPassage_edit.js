//=============================================================================
// TDDP_PlayerTouchPassage_edit.js
// Version: 1.0.1a  20160727: merusaiaが一部をコメントアウト（詳しくは日本語ヘルプを見てください）
//=============================================================================
 ...
 * =~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~=~
 *  * TDDP_PlayerTouchPassage_edit.js
 * Version: 1.0.1a  20160727: merusaiaが一部をコメントアウト
 * 
 * ↓ merusaiaが追記
 * 【概要】
 * マウスもしくはタッチインプットの使用時、
 ...
 *
 * 【_editの変更点】
 *  YEP_RegionEvents.jsとの併用を想定していたようなので、
 *  YEP_RegionEvents.jsプラグインを使わない人のために、以下の５行は、コメントしてあります。
 * 
 ...
 *  // }
 *
 *  あと、28行目、変なところに「*」が入っていたので、これをコメントしました。これがあると正常に動きません。。
 *  
 *  //* // ここのアスタリスクはいらないはず。by merusaia
 *  
 *  それが、元のTDDP_PlayerTouchPassage.jsと、TDDP_PlayerTouchPassage_edit.jsの違いです。
 *  それ以外は一緒です。
 * 
 * アップデートと簡易使用方法については、
