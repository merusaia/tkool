 ...
 * 2016/08/02: スキップモード中、メッセージ中の「/.」や「\|」を飛ばす機能を追加。ついでに、「/.」や「\|」待ち時間を変更可能に。（merusaia）
 * 2016/08/05: 現在。絶賛改良中…（merusaia）
 *
 * 【概要】
 ...
 * ・ボタン長押しで高速化を禁止するば、パラメータ「マップ早送り倍数」と「戦闘早送り倍数」を1にすることで実現できます。
 * 
 * などなど、ツクールMVデフォルトの様々な箇所を改変します。
 * 特にTachiさんのプラグインには、ユーザさんに極力ストレスが掛からないように、何度もテストプレイをされて調整された跡があり、
 * 快適操作に最適化されたTachiさんの高速化ノウハウを即導入できる、有能なプラグインかもしれません。
 * タッチ操作にも対応しています。
 * 
 * ※ただし、競合にはほんとうに気をつけてください。。(*_*)
 * 数多くのメソッドを上書き（※callなどで元メソッドを呼び出す追記ではなく、完全に上書き）しています。でもそうしないと実現できない機能が多い‥。
 * 使う前は、メソッド名（.prototype.*** = function()のところ）をチェックをして、他のプラグインと競合していないか確かめてから使ってください。
 * 
 * 【詳細】
 * ＜「早送り」機能について＞
 * ・早送りとは、ツクールのデフォルトで実装されている機能であり、
 *   「決定ボタン(okボタン、Enter/zキーのこと)」を押しっぱなしにしている時、戦闘・メッセージ送りなどが早くなる機能のことです（通常は２倍になる）。
 * ・このプラグインでは、早送りのスピード倍率をゲーム内変数で調整できます。
 * ・また、高速化されるボタンに、「画面タッチ押しっぱなし（スマホのみ）」が自動的に追加されます。
 ...
 *      それでも、予期せぬ動作が起こる場合は、これらのメソッドを呼び出しているプラグインをOFFにしてください。
 * 
 ...
 *    → 数値をそのまま上書きしないといけないので、競合対策は困難。デキる人、よかったら競合対策して公開してもらえると助かります。。
 * 
 *    ■ゲームオーバー制御（ランダムエンカウントの敵に負けても、ゲームオーバーにならず、指定したコモンイベントを呼ぶ処理）
 *     BattleManager.updateBattleEnd
 *    → 条件分岐内の処理追加なので、競合対策は困難。デキる人、よかったら競合対策して公開してもらえると助かります。。
 * 
 ...
 *    → デフォルトではHPが１／４以下の味方を１００％守る仕様ですが、それをパラメータで自由に変えられるように変更しています。
 *      条件分岐内の処理追加なので、競合対策は困難。デキる人、よかったら競合対策して公開してもらえると助かります。。
 * 
 * 【開発者用メモ】
 * プラグイン競合スクリプトの書き方：
 ...
 * 上書きするメソッド名 = function() {
 *   _上書きするメソッド名.apply(this, arguments); // 上書き後メソッドの最初に、上書き前メソッドを呼び出す。
 ...
 *	   _KeyboardConfig_Window_Options_addGeneralOptions.apply(this, arguments);                       // (3)上書き後メソッド内で、上書き前メソッドを呼び出す。
 ...
 *  余談: いずれはVisualStudioCodeにフォルダ内での同一メソッド定義の検索が実装され、競合チェックも簡単になるのでは…（たぶん）。
 *
 *
 * 【著作権について】
 * このプラグインはMITライセンスです。
 ...
 *     ・改変・再配布時は、著作情報と、以下のMITライセンス全文は消さないでください。よろしくお願いします。
 ...
 * 
 * @param 戦闘武器モーションスピード
 * @desc 初期値=3。ツクールデフォルトに対する、サイドビューキャラの武器・戦闘モーション速度です。ゲーム中は変更不可。
 * @default 3
 * 
 * @param 戦闘移動スピード
 * @desc 初期値=4。ツクールデフォルトに対する、サイドビューキャラの移動速度です。ゲーム中は変更不可。
 * @default 4
 * 
 * @param 戦闘アニメーションスピード
 ...
 * @desc 初期値=ON。ダッシュ(Shift)ボタンで、ユーザが任意でスキップできるかです。
 * @default ON
 *
 * @param ↓スキップ機能詳細
 * @desc スキップ時の有効/無効化と、スキップ倍率です。ゲーム中は変更不可。制御文字「\.」や「\|」、ウェイトもすっ飛ばします。
 * @default "↓整数を入力"
 * 
 * @param スキップスピード
 * @desc 初期値=10。早送り時に掛け算する、スキップ倍率です。ゲーム中は変更不可。制御文字「\.」、「\|」、ウェイトはすっ飛ばします。
 * @default 10
 * 
 * @param スキップ中かを格納するスイッチ番号
 * @desc 初期値=8（\V[8]）。スキップ中に自動的にON/OFFするスイッチ番号です。
 * @default 8
 * 
 * @param スキップ禁止中かを格納するスイッチ番号
 * @desc 初期値=9（\V[9]）。このスイッチをONだと、スキップボタンによるユーザのスキップを禁止します。
 * @default 9
 * 
 * 
 * 
 ...
 * @desc 初期値=0。（15が使いやすいです)。\V[n]が未定義か0の場合、デフォルトスピード倍率(前述)が入ります。
 * @default 0
 * 
 * @param メッセージスピードを格納する変数番号
 * @desc 初期値=0。（16が使いやすいです)。\V[n]が未定義か0の場合、デフォルトスピード倍率(前述)が入ります。
 * @default 0
 * 
 * @param イベント早送り倍数を格納する変数番号
 * @desc 初期値=0。（301が使いやすいです)。\V[n]が未定義や0だと「早送りイベントスピード」（前述）になります。
 * @default 0
 * 
 * @param 戦闘早送り倍数を格納する変数番号
 * @desc 初期値=0。（302が使いやすいです)。\V[n]が未定義や0だと各種スピード倍率（前述）になります。
 * @default 0
 *
 * @param スキップ倍数を格納する変数番号
 * @desc 初期値=0。（303が使いやすいです)。\V[n]が未定義や0だと「スキップスピード」（前述）になります。
 * @default 0
 * 
 * @param ＜スピードの変更、終わり＞
 * @desc 
 ...
 * @desc 初期値=OFF。競合対策。このプラグインで、パラメータ限界値を変更するかを設定します。他のプラグインを使う場合はONにしてください。
 * @default OFF
 * 
 * @param ↓パラメータ限界値の変更
 ...
 * @desc 初期値=ON。競合対策。OFFだと、デフォルトのようにゲームオーバになります。他のプラグインを使用する場合は、OFFにしてください。
 * @default ON
 * 
 * @param ランダムエンカウント時にスイッチをONにするか
 * @desc 初期値=ON。競合対策。OFFだと、ランダムエンカウント時は何もしません。他のプラグインを使用する場合は、OFFにしてください。
 * @default ON
 * 
 * 
 * @param ↓戦闘時の変数制御
 ...
 * 
 * @param 身代わり条件を変えるか
 * @desc 初期値=ON。ツクールデフォルト（HPが25％以下だと必ずかばう）にしたい場合は、OFFにしてください。
 * @default ON
 * 
 * @param ↓身代わり条件の変更
...
(function () {
    ...
    */
    ...
    var _MAPSPEED_DEFAULT = getParamNumber(['MAPSPEED_DEFAULT', 'イベントスピード'], 1, 100); // 整数
    ...
    var _SKIPSPEED_DEFAULT = getParamNumber(['_SKIPSPEED_DEFAULT', 'スキップスピード'], 1, 100); // 整数
    // ゲームスピードを持つ変数番号
    var _VarNo_BattleSpeed = getParamNumber(['VarNo_BattleSpeed', '戦闘スピードを格納する変数番号'], 1, 9999); // ツクール変数番号は、1～9999までのはず。
    var _VarNo_MessageSpeed = getParamNumber(['VarNo_MessageSpeed', 'メッセージスピードを格納する変数番号'], 1, 9999); // ツクール変数番号は、1～9999までのはず。
    ...
    var _VarNo_Rapid_MapRate = getParamNumber(['VarNo_Rapid_MapRate', 'イベント早送り倍数を格納する変数番号'], 1, 9999); // ツクール変数番号は、1～9999までのはず。
    ...
    /** デバッグ出力用変数。なぜかひんぱんにNaNになるのでチェック。後で消します。*/
    var _debaguVar = 10;
    ...
    */
    ...
    Game_Interpreter.prototype.command230 = function() {
        ...
        if(isSkipMode() === false){
            _Game_Interpreter_command230.apply(this, arguments); // スキップ中じゃないなら、元のメソッドを呼び出して、待つよ。
        }
    };
    // =========== ウェイトコマンドの追加定義、終 ===============================================
    ...
    Scene_Map.prototype.updateMainMultiply = function () {
        if(_isNoChangeSpeed_inThisPlugin === true){              // このプラグインのスピード変更を禁止するなら、
            return _Scene_Map_updateMainMultiply(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
            // デフォルトはこの一行。
            ...
            // merusaiaの追加行は下。
            // 1. スピードのデフォルトは1。マップの移動スピードは、戦闘スピードやメッセージスピードでは変更しない。
    ...
    Window_BattleLog.prototype.updateWaitCount = function () {
        if(_isNoChangeSpeed_inThisPlugin === true){              // このプラグインのスピード変更を禁止するなら、
            return _Window_BattleLog_updateWaitCount(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
    ...
    Scene_Map.prototype.encounterEffectSpeed = function () {
        if(_isNoChangeSpeed_inThisPlugin === true){              // このプラグインのスピード変更を禁止するなら、
            return _Scene_Map_encounterEffectSpeed(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
    ...
    Scene_Map.prototype.updateEncounterEffect = function () {
        if(_isNoChangeSpeed_inThisPlugin === true){              // このプラグインのスピード変更を禁止するなら、
            return _Scene_Map_updateEncounterEffect(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
    ...
    Scene_Map.prototype.startEncounterEffect = function () {
        if(_isNoChangeSpeed_inThisPlugin === true){              // このプラグインのスピード変更を禁止するなら、
            return _Scene_Map_startEncounterEffect(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
    ...
    Sprite_Animation.prototype.update = function () {
        if(_isNoChangeSpeed_inThisPlugin === true){              // このプラグインのスピード変更を禁止するなら、
            return _Sprite_Animation_update(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
            ...
            // merusaiaの追加行は下。
            // 1. スピードのデフォルトは3。戦闘スピード1-9が設定してあれば、それに2を割ったもの。
    ...
    // Tachiさんによる敵スプライト高速化。敵の状態異常付加・解除や、倒れエフェクトなどが高速化される。これは敵全体攻撃の爽快感にかなり重要。
    var _Sprite_Enemy_update = Sprite_Enemy.prototype.update;
    Sprite_Enemy.prototype.update = function () {
        if(_isNoChangeSpeed_inThisPlugin === true){              // このプラグインのスピード変更を禁止するなら、
            return _Sprite_Enemy_update(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
            ...
            if (this._enemy) {
                ...
                // merusaiaの追加行は下。
                // 1. スピードのデフォルト。戦闘スピード1-9が設定してあれば、それに3を割ったもの。
    ...
    // 武器モーションアニメ高速化。サイドビュー時は、この倍率が、通常攻撃時の爽快感にかなり影響する。
    var _Sprite_Weapon_animationWait = Sprite_Weapon.prototype.animationWait;
    Sprite_Weapon.prototype.animationWait = function () {
        if(_isNoChangeSpeed_inThisPlugin === true){              // このプラグインのスピード変更を禁止するなら、
            return _Sprite_Weapon_animationWait(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
            ...
            // merusaiaの追加行は下。
            // 1. スピードのデフォルトは4。戦闘スピード1-9が設定してあれば、それに2を割ったもの。
            var _speed = 4;
            // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
            if (_VarNo_BattleSpeed != null && _VarNo_BattleSpeed >= 1 && _VarNo_BattleSpeed < $dataSystem.variables.length) {
                var _gameSpeed = $gameVariables.value(_VarNo_BattleSpeed);
    ...
    // アクターモーション高速化。味方の状態異常付加・解除や、倒れエフェクトなどが高速化される。これは全体攻撃の爽快感にかなり重要。
    var _Sprite_Actor_motionSpeed = Sprite_Actor.prototype.motionSpeed;
    Sprite_Actor.prototype.motionSpeed = function () {
        if(_isNoChangeSpeed_inThisPlugin === true){              // このプラグインのスピード変更を禁止するなら、
            return _Sprite_Actor_motionSpeed(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
            // // Tachiさんによる追加は以下の一行。高速モード時は4倍。デフォルトは return 12;のみ。
            // if(Input.isPressed('ok') || Input.isPressed('shift') || TouchInput.isPressed()){return 3;}else{return 12;}

            // merusaiaの追加行は下。
            // 1. スピードのデフォルトは1。個人的に、ここは、ツクールのデフォルトの1でもいいと思う。
            //    戦闘スピード1-9が設定してあれば、それに4を割ったもの。
    ...
    // 戦闘中キャラクターの移動。アクターが、前に出る動作、後ろに下がる動作、のことだと思う。ここの高速化は、アクターの行動の爽快感にかなり重要。
    var _Sprite_Actor_updateMove = Sprite_Actor.prototype.updateMove;
    Sprite_Actor.prototype.updateMove = function () {
        if(_isNoChangeSpeed_inThisPlugin === true){              // このプラグインのスピード変更を禁止するなら、
            return _Sprite_Actor_updateMove(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
            ...
            if (!bitmap || bitmap.isReady()) {
                ...
                // merusaiaの追加行は下。
                // 1. スピードのデフォルトは4。個人的に、ここは、ツクールのデフォルト1は遅すぎると思う。
                //    戦闘スピード1-9が設定してあれば、それに1を割ったもの。
                var _speed = _BATTLE_MOVESPEED_DEFAULT;
                // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                if (_VarNo_BattleSpeed != null && _VarNo_BattleSpeed >= 1 && _VarNo_BattleSpeed < $dataSystem.variables.length) {
                    var _gameSpeed = $gameVariables.value(_VarNo_BattleSpeed);
                    _speed = Number.parseInt(_gameSpeed / 1);
                    // 変数の値に変な値が入っていないか、随時チェック。
    ...
    */
    Scene_Map.prototype.isFastForward = function () {
        if(_isNoChangeSpeed_inThisPlugin === true){              // このプラグインのスピード変更を禁止するなら、
            return  Scene_Map_isFastForward(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
            // tachiさんのデフォルトは次の二行。
            //   return (  $gameMap.isEventRunning() && !SceneManager.isSceneChanging() &&
            ...
            // 早送りモードかを返す。
            return _isFastForwardButtonPressed;
        }
    };
    // =============================================================================
    ...
    Game_Action.prototype.apply = function (target) {
        if(_isOnEffectDamageDrain === false){              // このプラグインで吸収技にもダメージエフェクトをつけるがOFFなら、
            return  _Game_Action_apply(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
    ...
    BattleManager.updateBattleEnd = function () {
        if(_isOnContinueBattleEnd === false){              // このプラグインで吸収技にもダメージエフェクトをつけるがOFFなら、
            return  _BattleManager_updateBattleEnd(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
            if (this.isBattleTest()) {
                ...
                if (this._canLose) {
                    ...
                    // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                    if (_SwitchNo_BattleLose != null && _SwitchNo_BattleLose >= 1 && _SwitchNo_BattleLose < $dataSystem.variables.length) {
    ...
    Game_Player.prototype.executeEncounter = function () {
        if(_isOnSwitchBattleEncount === false){              // このプラグインで吸収技にもダメージエフェクトをつけるがOFFなら、
            return  _Game_Player_executeEncounter(this, arguments); // 元のメソッドを呼び出すよ。.call(this, 引数1, 引数2, ...)よりapplyの方がコピペミスを防げていいよ。
        }else{
            if (!$gameMap.isEventRunning() && this._encounterCount <= 0) {
                ...
                if ($dataTroops[troopId]) {
                    // 変数番号を格納している、パラメータに変な値が入っていないか、随時チェック。
                    if (_SwitchNo_RandomEncount != null && _SwitchNo_RandomEncount >= 1 && _SwitchNo_RandomEncount < $dataSystem.variables.length) {
    ...
    BattleManager.checkSubstitute = function (target) {
        ...
        if(_isChangeDefault_Migawari === false){
            return _BattleManager_checkSubstitute.call(this); // 元のメソッドを呼び出すよ。
        }else{
            ...
            if (!this._action.isCertainHit()) {
                ...
                if (target.isAlive() && target._hp <= target.mhp * _Migawawri_Sareru_MaxHPPercent / 100) {
            ...
            // かばわないといけない状況か（targetがピンチか）を返す。
            return _isTargetPinch;
        }
    };
    ...
    BattleManager.applySubstitute = function (target) {
        ...
        if(_isChangeDefault_Migawari === false){
            return _BattleManager_applySubstitute.call(this); // 元のメソッドを呼び出すよ。
        }else{
            // 返り値の、ダメージを受けるバトラー。デフォルトは、かばわれる側（target）
            ...
            if (this.checkSubstitute(target)) {
                ...
                for (var i = 0; i < members.length; i++) {
                    if (members[i].isSubstitute()) {
                        substitute = members[i]; // 制御フラグ:身代わりがついている、パーティ番号が i 番目に若い仲間。
    ...
    Window_Message.prototype.processEscapeCharacter = function (code, textState) {
        ...
        var _waitFrameNum = (Number.parseInt(_WaitPeriodDef/getMessageSpeed()) || 0);
        switch (code) {
            case '.':
                if(isSkipMode() === false){                                   // スキップ中だと、待ちません。
                    ...
                    _waitFrameNum = (Number.parseInt(_waitFrameNum/getMessageSpeed())  || 0); // NaNや数値にならない場合も、0になります。±Infinityだけ例外処理が必要です。
            ...
            case '|':
                if(isSkipMode() === false){                                   // スキップ中だと、待ちません。
                    ...
                    this.startWait(ConvertEscapeCharacters(Number.parseInt(_WaitLineDef/getMessageSpeed()) ));   // 独自の方法で、このパラメータ分だけウェイトするよ（元のツクールもともとの待ち時間は無視するよ）
                }
                break;
            default:
    ...
    Window_Message.prototype.updateShowFast = function() {
        _Window_Message_updateShowFast.call(this); // 元のメソッドを呼び出すよ。
    ...
    Window_Message.prototype.updateInput = function() {
        var ret = _Window_Message_updateInput.call(this); // 元のメソッドを呼び出すよ。
    ...
    Window_ScrollText.prototype.scrollSpeed = function() {
        var ret = Window_ScrollText_scrollSpeed.call(this); // 元のメソッドを呼び出すよ
    ...
    Window_BattleLog.prototype.messageSpeed = function() {
        var ret = _Window_BattleLog_messageSpeed.call(this); // 元のメソッドを呼び出すよ。
