...
var parameter = PluginManager.parameters('Tachi_editByMerusaia_old');
...
Window_BattleLog.prototype.updateWaitCount = function() {
    if (this._waitCount > 0) {
        ...
        if (Input.isPressed('shift') || this.isFastForward())
        {
           ...
           //早送りボタン効果なし。
           //this._waitCount -= 20; // tachiさんのデフォルト。なぜ-20フレームなのかは不明。早過ぎると、ログが目で終えないほど小さくなってしまう可能性があるからです。デフォルトの_waitCountは不明。
        ...
        // 以下、早送り中でなくても早くしたい場合、else{}内の次の中から一つをコメントアウトして選ぶ。
            ...
            this._waitCount -= 2;   // 早送りでない場合を、だいぶ早く調整したかったみたいで、2フレーム(1/30秒)だけ減らす。
...
Sprite_Animation.prototype.update = function() {
    ...
    if(Input.isPressed('ok') || Input.isPressed('shift') || TouchInput.isPressed()){
        //this.updateMain(); // ここまでだと２倍
        //this.updateMain(); // ここまでだと３倍 // tachiさんのデフォルト。
        //this.updateMain(); // ここまでだと４倍
