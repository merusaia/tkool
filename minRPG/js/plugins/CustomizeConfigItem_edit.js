...
// ----------------------------------------------------------------------------
// Version
// 改変  2016/10/30 名前の変更: CustomizeConfigItem.js → CustomizeConfigItem_edit.js
//                   → z_PluginCommandTypeMissCheck.jsで、プラグインコマンドの文字列や引数タイプミスをまとめてエラーチェックするため、
//                     Game_Interpreter.prototype.pluginCommandのtrye～catch文を削除 （改変者：merusaia）
...
(function () {
    'use strict';
    var pluginName = 'CustomizeConfigItem_edit';
    ...
    Game_Interpreter.prototype.pluginCommandCustomizeConfigItem = function (command, args) {
        switch (getCommandName(command)) {
            ...
            case 'オプション任意項目の隠し解除' :
                ...
                break;
            // 以下、メルサイアが改変。一旦、例外を履くようにしてみます・
            default:
                throw new Error(); // 例外を吐きます。
        }
    };
